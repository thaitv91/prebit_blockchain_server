    ...
    public function actionIndex()
    {
        ...
        if ($model_bonus->load(Yii::$app->request->post())) {
            ...
            if($balance_wallettoken <= $amount){
                Yii::$app->getSession()->setFlash('error_bonus', 'Your request cannot be processed at this time. Please try again later!');
    		...
    		if($amount > $bonus_wallet){
    			Yii::$app->getSession()->setFlash('error_bonus', 'Your bonus wallet balance is not enough to perform this action!');
                return $this->render('index', ['model_bonus'=>$model_bonus, 'model_main'=>$model_main, 'user'=>$user, 'dataProvider' => $dataProvider, 'address_btc' => $address_btc, 'token' =>$user->token]);
    		}
            if($amount * $token > $user->token){
                Yii::$app->getSession()->setFlash('error_bonus', 'Your token balance is not enough to perform this action!');
                return $this->render('index', ['model_bonus'=>$model_bonus, 'model_main'=>$model_main, 'user'=>$user, 'dataProvider' => $dataProvider, 'address_btc' => $address_btc, 'token' =>$user->token]);
            }
            if($amount > ($amountsh->amountgh - $total_amount)){
                $minamount = $amountsh->amountsh - $total_amount;
                Yii::$app->getSession()->setFlash('error_bonus', 'The amount you can sh this month is '.$minamount.'Btc !');
                return $this->render('index', ['model_bonus'=>$model_bonus, 'model_main'=>$model_main, 'user'=>$user, 'dataProvider' => $dataProvider, 'address_btc' => $address_btc, 'token' =>$user->token]);
            }


            $model_bonus->user_id = $user->id;
		    $model_bonus->amount = $amount;
		    $model_bonus->status = GhTransfer::STATUS_BONUS;
		    $model_bonus->publish = GhTransfer::PUBLISH_NOACTIVE;
		    $model_bonus->created_at = time();
		    $model_bonus->updated_at = time();
		    if($model_bonus->save(false)){
                ...
                Yii::$app->getSession()->setFlash('success_bonus', 'GH completed! It will take some time for the funds to be credited to your Bitway wallet!');
        ...
        if ($model_main->load(Yii::$app->request->post())) {
            ...
            if($balance_wallettoken <= $amount){
                Yii::$app->getSession()->setFlash('error_main', 'Your request cannot be processed at this time. Please try again later!');
                return $this->render('index', ['model_bonus'=>$model_bonus, 'model_main'=>$model_main, 'user'=>$user, 'dataProvider' => $dataProvider, 'address_btc' => $address_btc, 'token' =>$user->token]);
            }
    		if($amount > $main_wallet){
    			Yii::$app->getSession()->setFlash('error_main', 'Your main wallet balance is not enough to perform this action!');
                return $this->render('index', ['model_bonus'=>$model_bonus, 'model_main'=>$model_main, 'user'=>$user, 'dataProvider' => $dataProvider, 'address_btc' => $address_btc, 'token' =>$user->token]);
    		}
            if($amount * $token > $user->token){
                Yii::$app->getSession()->setFlash('error_main', 'Your token balance is not enough to perform this action!');
                return $this->render('index', ['model_bonus'=>$model_bonus, 'model_main'=>$model_main, 'user'=>$user, 'dataProvider' => $dataProvider, 'address_btc' => $address_btc, 'token' =>$user->token]);
            }
            if($amount > ($amountsh->amountgh - $total_amount)){
                $minamount = $amountsh->amountsh - $total_amount;
                Yii::$app->getSession()->setFlash('error_main', 'The amount you can sh this month is '.$minamount.'Btc !');
                return $this->render('index', ['model_bonus'=>$model_bonus, 'model_main'=>$model_main, 'user'=>$user, 'dataProvider' => $dataProvider, 'address_btc' => $address_btc, 'token' =>$user->token]);
            }
		    ...
		    if($model_main->save()){
                ...
                Yii::$app->getSession()->setFlash('success_main', 'GH completed! It will take some time for the funds to be credited to your Bitway wallet!');
		    	return $this->render('index', ['model_bonus'=>$model_bonus, 'model_main'=>$model_main, 'user'=>$user, 'dataProvider' => $dataProvider, 'address_btc' => $address_btc, 'token' =>$user->token]);
		    }
        }    
