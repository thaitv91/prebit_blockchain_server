...
class Top10Controller extends FrontendController
{
    public function actionIndex()
    {
        if (Yii::$app->user->isGuest){            
            return $this->redirect('/login?redirect=' . Yii::$app->convert->redirect('top10/index'));
        }
        $this->canUser();
    ...
    public function actionUsersh(){
        ...
        $string_sh = '';
        foreach ($data_sh as $key => $value2) {
        	if($key < 3){
	            $user = User::findOne($value2);
	            $string_sh .= '<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
	                <div class="avatar-top top-'.$key.'">';
	                    if (!empty($user->avatar)) {
	                        $string_sh .= '<img src="'.Yii::$app->params['url_file'].'/uploads/users/'.$user->avatar.'" class="img-responsive img-1">';
	                    }else{
	                        $string_sh .= '<img src="'.Yii::$app->params['url_file'].'/uploads/users/default-avatar.jpg"'.'class="img-responsive img-1">';
	                    }                              
	                    $string_sh .= '<div class="set-top">';
	                        if ($key == 0) {
	                            $string_sh .= '<img src="'.Yii::$app->params['url_file'].'/images/top-1.png"'.'class="img-responsive img-2">';
	                        }
	                        elseif ($key == 1) {
	                            $string_sh .= '<img src="'.Yii::$app->params['url_file'].'/images/top-2.png"'.'class="img-responsive img-2">';
	                        }
	                        else{
	                            $string_sh .= '<img src="'.Yii::$app->params['url_file'].'/images/top-3.png"'.'class="img-responsive img-2">';
	                        } 
	                    $string_sh .= '</div>                            
	                </div>
        ...
        return $string_result = array($string_sh, $string_sh2);
    }
    ...
    public function actionUsertop(){
        ...
        $string_user = '';
        foreach ($data_value as $key => $lv23) {
            $user = User::findOne($lv23);
            if ($key < 3) {
            	 $string_user .= '<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4"><div class="avatar-top top-'.$key.'">';

	            if (!empty($user->avatar)) {
	                $string_user .='<img src="'.Yii::$app->params['url_file'].'/uploads/users/'.$user->avatar.'" class="img-responsive img-1">';
	            }else{
	                $string_user .= '<img src="'.Yii::$app->params['url_file'].'/uploads/users/default-avatar.jpg" class="img-responsive img-1">';
	            } 

	            $string_user .= '<div class="set-top">';
	            if ($key == 0) {
	                $string_user .= '<img src="'.Yii::$app->params['url_file'].'/images/top-1.png" class="img-responsive img-2">';
	            }
	            elseif ($key == 1) {
	                $string_user .= '<img src="'.Yii::$app->params['url_file'].'/images/top-2.png" class="img-responsive img-2">';
	            }
	            else{
	                $string_user .= '<img src="'.Yii::$app->params['url_file'].'/images/top-3.png" class="img-responsive img-2">';
	            } 
	            $string_user .= '</div></div><h2 class="title-range">'.$user->username.'</h2></div>';
            }
	    ...
	    foreach ($data_value as $key => $lv2) {
	     ...
	     $string_user2 .='</tbody>';
        return $string_result = array($string_user, $string_user2);
    }
    ...
    public function actionUsercharity(){
        ...
        foreach ($data_charity as $key => $value) {
            ...
            $key = $key + 1; 
            $key2 = $key -1;      
            $string_charity = '<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
                <div class="avatar-top top-'.$key2.'">';
                    if (!empty($user->avatar)) {
                        $string_charity .= '<img src="'.Yii::$app->params['url_file'].'/uploads/users/'.$user->avatar.'"'.'class="img-responsive img-1">';
                    }else{
                        $string_charity .= '<img src="'.Yii::$app->params['url_file'].'/uploads/users/default-avatar.jpg"'.'class="img-responsive img-1">';
                    }                          
                    $string_charity .= '<div class="set-top">';
                        if ($key == 1) {
                            $string_charity .= '<img src="'.Yii::$app->params['url_file'].'/images/top-1.png"'.'class="img-responsive img-2">';
                        }
                        elseif ($key == 2) {
                            $string_charity .= '<img src="'.Yii::$app->params['url_file'].'/images/top-2.png"'.'class="img-responsive img-2">';
                        }
                        else{
                            $string_charity .='<img src="'.Yii::$app->params['url_file'].'/images/top-3.png"'.'class="img-responsive img-2">';
                        }
                    $string_charity .= '</div></div><h2 class="title-range">'.$user->username.'</h2>
            </div>';
        }
        ...
        return $string_result = array($string_charity, $string_charity2);
    }
}
